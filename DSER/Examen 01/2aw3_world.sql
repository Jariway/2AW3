...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 17-10-2018 a las 10:56:20
...
--
-- Base de datos: `2aw3_world`
--
CREATE DATABASE IF NOT EXISTS `2aw3_world` DEFAULT CHARACTER SET utf32 COLLATE utf32_general_ci;
USE `2aw3_world`;
...
-- Procedimientos
--
DROP PROCEDURE IF EXISTS `spGetAllCities`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetAllCities` ()  NO SQL
SELECT * FROM city$$

DROP PROCEDURE IF EXISTS `spGetCiudadPorID`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetCiudadPorID` (IN `pidCiudad` INT)  NO SQL
SELECT
city.Population as population,
city.CountryCode as countrycode,
city.Name as cityname,
country.Name as cname,
country.Population as cpop,
country.Continent as Continent,
tcities.Name

FROM city
inner join country on country.Code = city.CountryCode
inner join city as tcities on tcities.ID = country.Capital
WHERE city.ID = pidCiudad$$

DROP PROCEDURE IF EXISTS `spGetPaisesPorContinente`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetPaisesPorContinente` (IN `ppais` VARCHAR(20))  NO SQL
SELECT *
FROM country
WHERE country.Continent LIKE ppais$$

DROP PROCEDURE IF EXISTS `spGetSpanishCountries`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetSpanishCountries` ()  NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spInsertCity` (IN `pname` VARCHAR(80), IN `pcode` VARCHAR(6), IN `ppopulation` INT)  NO SQL
INSERT INTO
city(Name,CountryCode,Population)
VALUES(pname,pcode,ppopulation)$$
