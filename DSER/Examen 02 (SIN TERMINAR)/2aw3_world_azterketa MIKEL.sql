...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 13-11-2018 a las 13:04:44
...
-- Procedimientos
--
DROP PROCEDURE IF EXISTS `spGetCityID`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetCityID` (IN `pname` INT)  NO SQL
SELECT city.ID
FROM city
WHERE city.Name LIKE(pname)$$

DROP PROCEDURE IF EXISTS `spGetCountries`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetCountries` ()  NO SQL
SELECT country.Name as nombre, country.Code as id
from country$$

DROP PROCEDURE IF EXISTS `spGetCountryPopulation`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetCountryPopulation` (IN `ccode` VARCHAR(9))  NO SQL
SELECT SUM(city.Population) as pop
FROM city
WHERE city.CountryCode = ccode$$

DROP PROCEDURE IF EXISTS `spGetNCities`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetNCities` (IN `pcode` VARCHAR(9))  NO SQL
SELECT COUNT(city.Name) as num
FROM city
WHERE city.CountryCode = pcode$$

DROP PROCEDURE IF EXISTS `spGetSingleCountryData`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetSingleCountryData` (IN `ccode` VARCHAR(9))  NO SQL
SELECT country.Code,country.Name,country.Continent,country.SurfaceArea,country.IndepYear,country.Population,country.LifeExpectancy,city.Name as ciudad
FROM country
INNER JOIN city on city.ID = country.Capital
WHERE country.Code = ccode$$

DROP PROCEDURE IF EXISTS `spModificar`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spModificar` (IN `pcode` VARCHAR(255), IN `pname` VARCHAR(255), IN `pcontinent` VARCHAR(255), IN `psurface` INT(255), IN `pyear` INT(255), IN `ppop` INT(255), IN `pexp` INT(255), IN `pcap` VARCHAR(255))  NO SQL
UPDATE country
	SET country.Code = pcode,country.Name = pname, country.Continent = pcontinent, country.SurfaceArea = psurface, country.IndepYear = pyear, country.Population = ppop, country.LifeExpectancy = pexp, country.Capital = (SELECT city.ID
 FROM city
 WHERE city.Name = pcap AND city.CountryCode = pcode)
 
 WHERE country.Code = pcode$$

DROP PROCEDURE IF EXISTS `spModificarSinCity`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spModificarSinCity` (IN `pcode` VARCHAR(255), IN `pcontinent` VARCHAR(255), IN `psurface` INT(255), IN `pyear` INT(255), IN `ppop` INT(255), IN `pexp` INT(255))  NO SQL
UPDATE country
	SET country.Code = pcode, country.Continent = pcontinent, country.SurfaceArea = psurface, country.IndepYear = pyear, country.Population = ppop, country.LifeExpectancy = pexp
 WHERE country.Code = pcode$$
...
('ESP', 'Spain', 'Europe', 505992.00, 1491, 39441700, 79.0, NULL),
